...
export default function shieldsPanelReducer (
  ...
  switch (action.type) {
    ...
    case shieldsPanelTypes.CONTENT_SCRIPTS_LOADED: {
      ...
      Promise.all([chrome.braveShields.getBraveShieldsEnabledAsync(action.url), chrome.braveShields.getCosmeticFilteringControlTypeAsync(action.url)])
        .then(([braveShieldsEnabled, cosmeticFilteringControlType]: [boolean, string]) => {
          const doCosmeticBlocking = braveShieldsEnabled && tabData.cosmeticFilteringFlag && (cosmeticFilteringControlType !== 'allow')
          if (doCosmeticBlocking) {
            const hide1pContent = (cosmeticFilteringControlType === 'block')
